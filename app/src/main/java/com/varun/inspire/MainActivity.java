...
public class MainActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toggle.syncState();
        /*Glide.with(this).load(R.drawable.bluesunset).asBitmap().into(new SimpleTarget<Bitmap>() {
            @Override
            public void onResourceReady(Bitmap resource, GlideAnimation<? super Bitmap> glideAnimation) {
                Drawable drawable = new BitmapDrawable(resource);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                    LinearLayout linearLayout= (LinearLayout) findViewById(R.id.sideBarLinearLayout);
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.nav_solution_feed) {
        ...
        } else if (id == R.id.nav_inspire_feed) {
            getSupportActionBar().setTitle("Feed");
            InspireFragment inspireFragment = new InspireFragment();
            FragmentManager fragmentManager = getSupportFragmentManager();
            for (int i = 0; i < fragmentManager.getBackStackEntryCount(); i++) {
            ...
            getSupportActionBar().setTitle("CBT");
            CBTFragment cbtFragment = new CBTFragment();
            FragmentManager fragmentManager = getSupportFragmentManager();
            for (int i = 0; i < fragmentManager.getBackStackEntryCount(); i++) {
                fragmentManager.popBackStack();
            }
            fragmentManager.beginTransaction().replace(R.id.content_main, cbtFragment).commit();
        }
